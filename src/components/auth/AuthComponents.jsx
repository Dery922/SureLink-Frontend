...
export function FeedbackBanner({ type = "error", message, onRetry }) {
  ...
  return (
    <div
      className={`border rounded-xl p-3.5 text-sm flex items-start gap-2.5 transition-all ${palette[type] || palette.error}`}
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {onRetry && (
          <button
            type="button"
            onClick={onRetry}
            className="text-xs font-semibold underline mt-1 hover:opacity-80 transition-opacity"
          >
            Retry
          </button>
        )}
...
export function AuthButton({
  ...
  const base =
    "w-full py-3 rounded-xl font-bold text-sm transition-all active:scale-[0.97] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2";
  const stylesMap = {
    primary:
      ...
      {Array.from({ length: 6 }).map((_, idx) => {
        ...
        return (
          <input
            ...
            value={safe[idx] || ""}
            onChange={(e) =>
              updateAt(idx, e.target.value.replace(/\D/g, "").slice(-1))
            }
            ...
            /* Keeping your exact dimensions, background, and dark-mode styling untouched */
            className="w-11 h-13 text-center text-lg font-bold border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 rounded-xl focus:outline-none focus:ring-4 focus:ring-[#FFCC00]/50 focus:border-[#0057FF] transition-all input-glow text-gray-900 dark:text-gray-100"
