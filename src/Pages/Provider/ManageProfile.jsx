...
function ManageProfile() {
  ...
  // New function for confirming gallery deletion
  const confirmGalleryDelete = async () => {
    if (!galleryImageToDelete) return;

    try {
      setGalleryLoading(true);
      const response = deleteGalleryImage(galleryImageToDelete._id);

      if (response.success) {
        setGallery((prev) =>
          prev.filter((item) => item._id !== galleryImageToDelete._id),
        );
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      // ✅ CORRECT: Extract the ID properly
      const serviceId = serviceToDelete._id || serviceToDelete.id;

      console.log("🗑️ Service ID passed to confirmDeleteService:", serviceId);
      ...
      if (response && response.success) {
        ...
        setServices(services.filter((s) => (s.id || s._id) !== serviceId));
  ...
  const handleSave = async () => {
    try {
      ...
      error("error", "Failed to save profile. Please try again");
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="bg-[#F8FAFB] min-h-screen">
        <NavbarDashbaord />
        <div className="pt-[120px] pb-16 flex items-center justify-center min-h-[60vh]">
    ...
    <>
      <div className="bg-[#F8FAFB] min-h-screen">
                                  ...
                                  <div className="flex gap-1 ml-3 flex-shrink-0">
                                    ...
                                    <button
                                      onClick={() => {
                                        ...
                                        // Set the service to delete and show the modal
        ...
        <ConfirmationModal
          ...
          message={`Are you sure you want to delete "${galleryImageToDelete?.title || "this image"}"? This action cannot be undone.`}
