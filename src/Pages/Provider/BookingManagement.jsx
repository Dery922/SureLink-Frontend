...
import { acceptBooking } from "../../services/services";

// Quick Cancel Reasons - Just tap and confirm!
const CANCEL_REASONS = {
  provider: [
    ...
    { id: "weather", label: "Weather Issue", icon: FaCloudRain, color: "blue" },
    {
      id: "client_request",
...
const BookingManagement = () => {
  ...
  const tabs = [
    ...
    },
    {
      ...
      label: "History",
      icon: FaHistory,
      count: stats.completed + stats.cancelled + stats.no_show,
    },
  ];
        ...
        if (response.data.stats) {
          ...
          setStats({
            ...
            cancelled: bookingsData.filter((b) => b.status === "cancelled")
              .length,
            no_show: bookingsData.filter((b) => b.status === "no_show").length,
            rescheduled: bookingsData.filter((b) => b.status === "rescheduled")
              .length,
          });
        }
      } else {
  ...
  // Determine role based on booking data
  const getBookingRole = (booking) => {
    if (!booking || !user) return "unknown";
    if (booking.customerId === user?._id || booking.customerId === user?.id) {
      return "customer";
    }
    if (booking.providerId === user?._id || booking.providerId === user?.id) {
      return "provider";
    }
    return "unknown";
  };

  // Get cancel reasons for role
  const getCancelReasonsForRole = (booking) => {
    const role = getBookingRole(booking);
    return role === "provider"
      ? CANCEL_REASONS.provider
      : CANCEL_REASONS.customer;
  };

  // Handle Confirm Booking
  const handleConfirmBooking = (booking) => {
    setConfirmBookingData(booking);
  ...
  const handleConfirmBookingConfirm = async () => {
    ...
    try {
      const response = await acceptBooking(confirmBookingData._id);
      if (response.data.success) {
  ...
  // Handle Cancel Booking - Send reason to backend
  const handleCancelBookingConfirm = async () => {
    ...
    try {
      ...
      // Find the selected reason object to get the label
      const booking = bookings.find((b) => b._id === cancelBookingId);
      const reasons = getCancelReasonsForRole(booking);
      const reasonObj = reasons.find((r) => r.id === selectedCancelReason);
      const reasonText = reasonObj?.label || selectedCancelReason;

      // Send cancellation with reason to backend
      const response = await api.put(
        `/bookings/${cancelBookingId}/cancel`,
        {
          reason: reasonText, // This will be stored in cancelledReason field
          cancelReasonType: selectedCancelReason, // Additional metadata if needed
        },
        {
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (response.data.success) {
        toast.success(`Booking cancelled: ${reasonText}`);
        setShowCancelModal(false);
        setCancelBookingId(null);
        setSelectedCancelReason(null);
        setCancelReason("");
        fetchBookings(); // Refresh to show updated status and reason
      } else {
  ...
  // Handle Confirm Action (Start/Complete)
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      switch (confirmAction) {
        case "start":
          endpoint = `/bookings/${confirmBookingId}/start`;
          successMessage = "Booking started!";
          break;
        case "complete":
          endpoint = `/bookings/${confirmBookingId}/complete`;
  ...
  const renderBookingTicket = (booking) => {
    ...
    const customerAvatar =
      ...
      "&background=0057FF&color=fff&size=64";
    ...
    const roleBadge = isProviderRole
      ? {
          label: "You are the Provider",
          color: "bg-blue-100 text-blue-700",
          icon: FaStore,
        }
      : isCustomerRole
        ? {
            label: "You are the Customer",
    ...
    const displayName = isProviderRole ? customerName : providerName;
    const displayAvatar = isProviderRole ? customerAvatar : providerAvatar;
    const displayRole = isProviderRole ? "Customer" : "Provider";
        ...
        <div className="p-5 md:p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span
                className={`px-3 py-1 rounded-full text-xs font-medium ${roleBadge.color} flex items-center gap-1.5`}
              >
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-600 flex items-center gap-1">
                    <FaUser className="text-[#0057FF] text-xs" />
              ...
              <div className="flex flex-wrap gap-2">
                  ...
                  isProviderRole && (
                    <>
                      <button
                        ...
                        className="px-4 py-2 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white text-sm font-semibold rounded-xl hover:shadow-lg hover:scale-105 transition-all duration-300 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {actionLoading ? (
                      ...
                      <button
                        ...
                        disabled={actionLoading}
                        className="px-4 py-2 border border-gray-300 text-gray-600 text-sm font-semibold rounded-xl hover:border-red-300 hover:text-red-500 hover:bg-red-50 transition-all duration-300 flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                ...
                {isOutgoing &&
                  ...
                  isCustomerRole && (
                    <button
                      onClick={() => handleCancelBooking(booking._id)}
                      disabled={actionLoading}
                      className="px-4 py-2 border border-red-200 text-red-500 text-sm font-semibold rounded-xl hover:bg-red-50 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {actionLoading ? (
                        <FaSpinner className="animate-spin" />
                      ) : (
                ...
                {booking.status === "confirmed" && (
                  <button
                    ...
                    disabled={actionLoading}
                    className="px-4 py-2 bg-gradient-to-r from-blue-500 to-indigo-500 text-white text-sm font-semibold rounded-xl hover:shadow-lg hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {actionLoading ? (
                      <FaSpinner className="animate-spin" />
                    ) : (
                ...
                {(booking.status === "confirmed" ||
                  ...
                  <button
                    ...
                    disabled={actionLoading}
                    className="px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-sm font-semibold rounded-xl hover:shadow-lg hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {actionLoading ? (
                      <FaSpinner className="animate-spin" />
                    ) : (
          ...
          {/* Bottom - Additional Info - Show cancellation reason prominently */}
          <div className="mt-4 pt-4 border-t border-gray-100 flex flex-wrap items-center gap-4 text-xs text-gray-500">
            ...
            {/* Display cancellation reason prominently if exists */}
            {booking.cancelledReason && (
              <span className="flex items-center gap-2 bg-red-50 border border-red-200 px-3 py-1.5 rounded-full text-red-700 font-medium">
    ...
    <div className="bg-gradient-to-br from-[#F5F8FF] via-white to-[#F5F8FF] min-h-screen">
      ...
      <div className="max-w-[1280px] mx-auto px-5 pt-[100px] pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-500 mt-1">
              Manage bookings you receive and ones you make
            </p>
          </div>
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl p-4 mb-6">
          <div className="flex items-center gap-3">
            <FaExchangeAlt className="text-[#0057FF] text-xl" />
            <div>
              <p className="text-sm font-medium text-gray-800">
                ...
                active bookings across all roles
              </p>
              <p className="text-xs text-gray-500 mt-0.5">
                View all bookings you receive and ones you've made across
                different tabs
              </p>
            </div>
          </div>
        ...
        {loading ? (
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-16 text-center">
            ...
            <p className="text-gray-400">
              {activeTab === "incoming" &&
                "You don't have any incoming bookings as a provider"}
              {activeTab === "outgoing" &&
                "You haven't made any bookings as a customer"}
              {activeTab === "history" && "No booking history found"}
            </p>
          </div>
        ) : (
      ...
      {showConfirmBookingModal && confirmBookingData && (
          ...
          <div className="min-h-screen px-4 flex items-center justify-center">
            <div className="relative bg-white rounded-3xl max-w-md w-full shadow-2xl animate-scaleIn p-6">
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-800">
                      {confirmBookingData.serviceName || "N/A"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Customer</span>
                    <span className="font-medium text-gray-800">
                      {confirmBookingData.customerName || "N/A"}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Total Amount</span>
                    <span className="font-medium text-[#0057FF]">
                      {formatCurrency(confirmBookingData.totalAmount)}
                    </span>
                  </div>
                  {confirmBookingData.bookingDate && (
                    <div className="flex justify-between">
                  ...
                  {confirmBookingData.bookingTime && (
                    <div className="flex justify-between">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 mb-6">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-xs text-amber-700">
                    By confirming this booking, you agree to provide the service
                    as scheduled. This action cannot be undone.
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={handleConfirmBookingConfirm}
                  disabled={actionLoading}
                  className="flex-1 px-4 py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-xl hover:shadow-lg transition-all font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {actionLoading ? (
                    <FaSpinner className="animate-spin" />
                  ) : (
      ...
      {/* QUICK CANCEL MODAL - Just tap a reason and confirm! */}
      {showCancelModal && (
        <div className="fixed inset-0 z-[100] overflow-y-auto">
          <div
            className="fixed inset-0 bg-black/50 backdrop-blur-sm"
          ...
          />
          <div className="min-h-screen px-4 flex items-center justify-center">
            <div className="relative bg-white rounded-3xl max-w-2xl w-full shadow-2xl animate-scaleIn p-6">
              <button
                onClick={() => {
                  setShowCancelModal(false);
                  setCancelBookingId(null);
                ...
                className="absolute top-4 right-4 w-10 h-10 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors flex items-center justify-center z-10"
              >
                <FaTimes className="text-xl text-gray-500" />
              </button>
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Tap a reason below to cancel this booking instantly
                </p>
              </div>

              {/* Quick Reason Chips - Just tap and go! */}
              <div className="mb-6">
                <div className="flex flex-wrap gap-3 justify-center">
                  {getCancelReasonsForRole(
                    bookings.find((b) => b._id === cancelBookingId),
                  ).map((reason) => {
                    const Icon = reason.icon;
                    const isSelected = selectedCancelReason === reason.id;
                    const colorMap = {
                      amber:
                        "border-amber-200 bg-amber-50 hover:bg-amber-100 text-amber-700 hover:border-amber-400",
                      red: "border-red-200 bg-red-50 hover:bg-red-100 text-red-700 hover:border-red-400",
                      orange:
                        "border-orange-200 bg-orange-50 hover:bg-orange-100 text-orange-700 hover:border-orange-400",
                      purple:
                        "border-purple-200 bg-purple-50 hover:bg-purple-100 text-purple-700 hover:border-purple-400",
                      indigo:
                        "border-indigo-200 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 hover:border-indigo-400",
                      blue: "border-blue-200 bg-blue-50 hover:bg-blue-100 text-blue-700 hover:border-blue-400",
                      cyan: "border-cyan-200 bg-cyan-50 hover:bg-cyan-100 text-cyan-700 hover:border-cyan-400",
                    };
                    ...
                    return (
                      <button
                        ...
                        className={`px-6 py-3 rounded-2xl border-2 text-sm font-medium transition-all duration-200 flex items-center gap-2 ${
                          colorMap[reason.color] ||
                          "border-gray-200 bg-gray-50 hover:bg-gray-100 text-gray-700"
                        } ${
                          isSelected
                            ? "ring-4 ring-offset-2 ring-blue-400 border-blue-500 shadow-lg transform scale-105"
                            : "hover:scale-105"
                        }`}
                      >
              ...
              {selectedCancelReason && (
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-3 mb-6 animate-fadeInUp">
                  <div className="flex items-center justify-between">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-600 mt-0.5">
                      {getBookingRole(
                        ...
                        : "The service provider will be notified of the cancellation."}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={actionLoading || !selectedCancelReason}
        ...
        <div className="fixed inset-0 z-[100] overflow-y-auto">
          ...
          <div className="min-h-screen px-4 flex items-center justify-center">
            <div className="relative bg-white rounded-3xl max-w-md w-full shadow-2xl animate-scaleIn p-6">
              ...
              <div className="text-center mb-6">
                <div
                  className={`w-16 h-16 ${confirmAction === "start" ? "bg-blue-100" : "bg-emerald-100"} rounded-full flex items-center justify-center mx-auto mb-4`}
                >
                  {confirmAction === "start" ? (
                    <FaClockIcon className="text-3xl text-blue-500" />
                  ) : (
                    <FaCheckCircle className="text-3xl text-emerald-500" />
                  )}
                </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={actionLoading}
                  className={`flex-1 px-4 py-3 ${confirmAction === "start" ? "bg-blue-500 hover:bg-blue-600" : "bg-emerald-500 hover:bg-emerald-600"} text-white rounded-xl transition-all font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2`}
                >
                  {actionLoading ? (
                    <FaSpinner className="animate-spin" />
                  ) : confirmAction === "start" ? (
          ...
          <div className="min-h-screen px-4 flex items-center justify-center">
            <div className="relative bg-white rounded-3xl max-w-3xl w-full shadow-2xl animate-scaleIn p-6 md:p-8">
              ...
              <div className="space-y-6">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm opacity-75">
                        {getBookingRole(selectedBooking) === "provider"
                ...
                {/* Show cancellation reason prominently if booking is cancelled */}
                  ...
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <div className="flex items-center gap-3">
                      <img
                        ...
                        alt={selectedBooking.customerName || "Customer"}
                      ...
                      <div>
                        <p className="font-semibold text-gray-800">
                          {selectedBooking.customerName || "Unknown"}
                        </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                    ...
                    {selectedBooking.rescheduledAt && (
                      <div className="flex justify-between py-1 border-b border-gray-200">
                        ...
                        <span className="font-medium text-gray-700">
                          {formatDate(selectedBooking.rescheduledAt)}
                        </span>
                      </div>
                    )}
              ...
              <div className="flex flex-wrap gap-3 mt-6 pt-4 border-t border-gray-200">
                  ...
                  getBookingRole(selectedBooking) === "provider" && (
                    <>
                      <button
                        ...
                        disabled={actionLoading}
                        className="flex-1 min-w-[100px] px-4 py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-xl hover:shadow-lg transition-all font-medium disabled:opacity-50"
                      >
                        {actionLoading ? (
                          <FaSpinner className="animate-spin" />
                        ) : (
                ...
                {selectedBooking.status === "pending" &&
                  getBookingRole(selectedBooking) === "customer" && (
                    <button
                      onClick={() => {
                        handleCancelBooking(selectedBooking._id);
                        setShowDetailsModal(false);
                ...
                {selectedBooking.status === "confirmed" && (
                  <button
                    onClick={() => {
                      handleStartBooking(selectedBooking._id);
                      setShowDetailsModal(false);
                    }}
                    disabled={actionLoading}
                    className="flex-1 min-w-[100px] px-4 py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-xl hover:shadow-lg transition-all font-medium disabled:opacity-50"
                  >
                    {actionLoading ? (
                      <FaSpinner className="animate-spin" />
                    ) : (
                ...
                {(selectedBooking.status === "confirmed" ||
                  ...
                  <button
                    ...
                    disabled={actionLoading}
                    className="flex-1 min-w-[100px] px-4 py-3 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl hover:shadow-lg transition-all font-medium disabled:opacity-50"
                  >
                    {actionLoading ? (
                      <FaSpinner className="animate-spin" />
                    ) : (
